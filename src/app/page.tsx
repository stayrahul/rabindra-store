...
export default function HomePage() {
  ...
  const handleAddToCart = (e: React.MouseEvent, product: Product) => {
    ...
    const price = Number(activeVariant?.price) || Number(productData?.sellingPrice) || 0;

    dispatch(
      addToCart({
        id: `${product.id}-${defaultUnit}-${Date.now()}`,
        name: product.nameEn,
      ...
      <main className="max-w-[90rem] mx-auto p-4 md:p-8 pb-24">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 mt-4 md:mt-8">
          <div>
            ...
            <p className="text-slate-500 mt-2 text-base md:text-lg font-medium">Live stock and wholesale rates for Rabindra Store.</p>
          </div>
          ...
          >
            <AnimatePresence mode="popLayout">
              {filteredProducts.map((product, i) => {
                ...
                const variantsArray = productData?.variants || productData?.varients || [];
                const activeVariant = variantsArray.find((v: any) => v.unit === defaultUnit);
                const displayPrice = Number(activeVariant?.price) || Number(productData?.sellingPrice) || 0;
                  ...
                  >
                    ...
                    <div className="p-4 md:p-5 flex flex-col flex-1">
                      <Link href={`/product/${product.id}`} className="block group/text">
                        ...
                        <h2 className="font-black text-slate-900 text-base md:text-lg leading-tight line-clamp-1 group-hover/text:text-indigo-600 transition-colors">
                          {product.nameEn}
                        </h2>
                        <p className="text-xs font-bold text-slate-400 mt-0.5 line-clamp-1 font-hindi">
                          {product.nameNp || "—"}
                        </p>
                      </Link>

                      {/* --- UPGRADED: PRICING BAR --- */}
                      <div className="flex items-center justify-between mb-4 mt-4 bg-slate-50 rounded-xl p-2.5 border border-slate-100/50">
                        <div className="flex flex-col">
                           <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Price</span>
                           <span className="text-xs font-black text-slate-900">
                             {displayPrice > 0 ? `Rs. ${displayPrice.toLocaleString('en-IN')}` : "On Request"}
                           </span>
                        </div>
                        <div className="h-6 w-[1px] bg-slate-200"></div>
                        <div className="flex flex-col items-end text-right">
                           <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Unit</span>
                      ...
                      <div className="mt-auto">
                        <button 
                          ...
                          className={`w-full font-bold py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 active:scale-95 text-sm ${
